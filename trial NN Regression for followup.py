...
def batch_run(LR, WD, ITERATION, LR_ADJUST_START, MODEL):
    ...
    for tr_idx, ts_idx in kf.split(X):
        ...
        scheduler1 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer1, 'min', patience=500, factor=0.95, verbose=True)
        scheduler2 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer2, 'min', patience=500, factor=0.95, verbose=True)
        ...
        preds = Variable(ys_tr).to(device)

        for i in range(ITERATION):
            prediction = net1(inputs)
            loss1 = loss_func(prediction, preds)
        ...
        inputs = Variable(X_tr).to(device)
        preds = Variable(el_tr).to(device)

        for i in range(ITERATION):
            prediction = net2(inputs)
            loss2 = loss_func(prediction, preds)
    ...
    with open(result_filename, 'a') as f:
        ...
        f.write(f'\nLearning rate: {LR}\n, Weight decay: {WD}\n, LR adjust start: {LR_ADJUST_START}\n')
        f.write('\nOverall Results\n')
    ...
    #
    #     plt.savefig('pred/' + MODEL + '_G1_plot_y' + str(yi), bbox_inches='tight')
    #     # plt.show()
    #     plt.close()
    #
    ...
    #     plt.ylabel("Values")
    #
    #     plt.savefig('pred/' + MODEL + '_G2_plot_y' + str(yi), bbox_inches='tight')
    #     # plt.show()
    #     plt.close()
...
for wd in [0.01]:
    for lr in [0.0001]:
        for lr_adjust in [500]:
            for r in range(1):
                batch_run(LR=lr, WD=wd, ITERATION=500, LR_ADJUST_START=lr_adjust, MODEL=model)